...
static uint8_t protocol_frame_parse(uint8_t *data, uint16_t *data_len) {
  ...
  need_to_parse_len = recvbuf_get_len_to_parse(parser.frame_len, PROT_FRAME_LEN_RECV, parser.r_oft,
  ...
  if ((parser.frame_len + parser.r_oft - PROT_FRAME_LEN_CHECKSUM) > PROT_FRAME_LEN_RECV) {
    ...
    checksum = check_sum(checksum, parser.recv_ptr,
                         parser.frame_len - PROT_FRAME_LEN_CHECKSUM + parser.r_oft - PROT_FRAME_LEN_RECV);
  } else {
    /* 数据帧可以一次性取完*/
  ...
  while (1) {
    ...
    switch (cmd_type) {
      ...
      case SET_TARGET_CMD: {
        ...
        /* 在这里填写自己的处理函数 */
        set_pid_target_val_cmd(target_temp);
      } break;
...
WEAK void set_pid_paramter_cmd(float p, float i, float d) {}

WEAK void set_pid_target_val_cmd(int target_val) {}

WEAK void set_pid_period_cmd(uint32_t period) {}

WEAK void pid_start_cmd(void) {}

WEAK void pid_stop_cmd(void) {}

WEAK void pid_reset_cmd(void) {}

WEAK void port_send_data_to_computer(void *data, uint8_t num);
